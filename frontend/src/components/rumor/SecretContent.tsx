...
export const SecretContent: React.FC<Props> = ({
    ...
    return (
        <div className={`relative overflow-hidden rounded-xl border-4 transition-all duration-500 ${isRevealed ? 'border-pop-green bg-white shadow-none' : 'border-pop-black bg-gray-100 shadow-hard'
            }`}>

            {/* Status Bar */}
            <div className={`p-4 border-b-4 border-pop-black flex justify-between items-center ${isRevealed ? 'bg-pop-green text-white' : 'bg-pop-black text-white'
                }`}>
                <h3 className="font-black uppercase tracking-widest text-lg flex items-center gap-2">
                    {isRevealed ? t('secret.decrypted_title') : t('secret.encrypted_title')}
                </h3>
                ...
                {isRevealed && contentUrl ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center z-10 py-12">
                        ...
                        <h4 className="text-4xl font-black text-pop-black mb-4 uppercase tracking-tight">
                            {canReveal ? t('secret.access_granted') : t('secret.top_secret')}
                        </h4>

                        <p className="text-lg font-bold text-gray-500 max-w-md mb-8 leading-relaxed">
                            {canReveal
                                ? t('secret.reveal_desc')
                                : t('secret.locked_desc')}
                        </p>
                        ...
                        {canReveal && (
                            ...
                            >
                                {isDecrypting ? t('secret.decrypting') : t('secret.reveal_button')}
                            </Button>
                        )}
                    </div>
                )}
